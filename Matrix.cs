...
namespace Mapack
{
	...
	public class Matrix
	{
		...
		//Added by Christofer Berruz Chungata

		public Matrix HadamardProduct(Matrix B)
		{
			if (columns != B.Columns || rows != B.Rows) { throw new ArgumentException("Matrix dimensions are not valid"); }
			Matrix X = new Matrix(rows, columns);
			for(int i = 0; i < rows; i++)
			{
				for(int j = 0; j < columns; j++)
				{
					X[i, j] = this.data[i][j] * B[i, j];
				}
			}
			return X;
		}
